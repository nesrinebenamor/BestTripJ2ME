 ...
 *
 */
public class FileBrowserClasse extends Form implements CommandListener {
    ...
    public FileBrowserClasse(int id,Display disp) {
        super("test");
        this.disp=disp;
        this.id=id;
        this.startApp();
    ...
    public FileBrowserClasse(String login)
    {
        super("test");
        this.login=login;
    }
    ...
    public void commandAction(Command c, Displayable d) {
     ...
     public void recupererMotdepasse(int id)
    {    try {
            ...
            HttpConnection hc = (HttpConnection) Connector.open("http://localhost/parsingBest/recuperer.php?"+"id="+id);
            ...
            personnes = personnesHandler.getUtilisateur();  
             ...
             if (personnes.length > 0) {
                for (int i = 0; i < personnes.length; i++) {    
                    ...
                    username2=personnes[i].getUsername();
                    age=personnes[i].getAge1();
                    password=personnes[i].getPassword();
                        
             utils.StaticMidlet.disp.setCurrent(new Alert("Connection correct", "Vous allez changer d'image", null, AlertType.INFO),new Menumezyen(personnes[0].getId()));
                }
            }
    ...
    public void inserer(String image) {
        System.out.println("id est celui la"+id);
        try {
            hc = (HttpConnection) Connector.open("http://localhost/parsingBest/ajoutImage.php?image="+image+"&id="+id);
            ...
            if ("OK".equals(sb.toString().trim())) {
                ...
                //disp.setCurrent(f2);
     utils.StaticMidlet.disp.setCurrent(new Alert("Votre image a été inséré avec succés", "", null, AlertType.INFO));
                utils.StaticMidlet.disp.setCurrent(new Menumezyen(id));
            } else {
                disp.setCurrent(f3);
     utils.StaticMidlet.disp.setCurrent(new Alert("Erreur lors de l'insertion", "", null, AlertType.ERROR));
